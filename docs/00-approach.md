...
step 6 renders once, measures the median residual per font size against the reference, and
writes it to `calibration.json`. The second render applies it. The correction is a
measurement, and it is committed next to the template so a reviewer can see it.
...
* **The rendered PDF is the deliverable.** Per-page images of a passing render are not
  committed; diff images are written only for pages that fail, which is when a reviewer needs
  them.
